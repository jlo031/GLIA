...
# -------------------------------------------------------------------------- #
    ...
    n_class = int(clf_params_dict['n_class'])
    n_feat  = int(clf_params_dict['n_feat'])

    logger.info(f'clf_type: {clf_type}')
    logger.info(f'required_features: {required_features}')
    logger.info(f'Classifier has {n_class} classes and {n_feat} features')
